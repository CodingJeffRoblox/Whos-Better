...
function renderHallOfFame() {
    ...
    const topItems = items.sort((a, b) => b.winRate - a.winRate).slice(0, 25);
    ...
    if (topItems.length === 0) {
        container.innerHTML = `
            <div style="grid-column: 1/-1; text-align: center; padding: 3rem; color: rgba(255,255,255,0.5);">
                ...
                <div>Start voting on the homepage to fill the Hall of Fame!</div>
            </div>
        `;
        return;
    }
    ...
    container.innerHTML = topItems.map((item, index) => `
        <div class="hall-item" style="animation: fadeInUp 0.5s ease ${index * 0.05}s both;">
            <div class="hall-rank" style="color: ${index < 3 ? '#fbbf24' : item.color};">
            ...
            <div class="hall-info">
                ...
                <div class="hall-stats">
                    <span style="color: #22c55e; font-weight: 700;">${item.winRate}%</span> win rate • ${item.total} votes
                </div>
            </div>
        </div>
    `).join('');
