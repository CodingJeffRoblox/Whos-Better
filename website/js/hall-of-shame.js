...
function renderHallOfShame() {
    ...
    const bottomItems = items.sort((a, b) => a.winRate - b.winRate).slice(0, 25);
    
    const container = document.getElementById('hallOfShame');
    
    if (bottomItems.length === 0) {
        container.innerHTML = `
            <div style="grid-column: 1/-1; text-align: center; padding: 3rem; color: rgba(255,255,255,0.5);">
                <div style="font-size: 4rem; margin-bottom: 1rem;">💀</div>
                <div style="font-size: 1.5rem; margin-bottom: 0.5rem;">No votes yet</div>
                <div>Start voting on the homepage to fill the Hall of Shame!</div>
            </div>
        `;
        return;
    }
    
    container.innerHTML = bottomItems.map((item, index) => `
        <div class="hall-item" style="animation: fadeInUp 0.5s ease ${index * 0.05}s both;">
            <div class="hall-rank" style="color: ${index < 3 ? '#ef4444' : item.color};">
                ${index === 0 ? '💀' : index === 1 ? '😢' : index === 2 ? '😭' : `#${index + 1}`}
            </div>
            <div class="hall-image" style="background: ${item.color}; display: flex; align-items: center; justify-content: center; font-size: 2rem; font-weight: 700;">
                ${item.name.charAt(0)}
            </div>
            <div class="hall-info">
                <div class="hall-name" style="color: ${item.color};">${item.name}</div>
                <div class="hall-stats">
                    <span style="color: #ef4444; font-weight: 700;">${item.winRate}%</span> win rate • ${item.total} votes
                </div>
            </div>
        </div>
    `).join('');
